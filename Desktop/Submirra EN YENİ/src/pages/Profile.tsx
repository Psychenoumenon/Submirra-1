...
export default function Profile() {
  ...
  const checkReportStatus = async (targetUserId: string) => {
    if (!user) return;
  ...
  const handleWithdrawReport = async () => {
    ...
    if (!confirm(language === 'tr' ? 'Şikayetinizi geri çekmek istediğinizden emin misiniz?' : 'Are you sure you want to withdraw your report?')) {
      return;
    }

    try {
      // First, find the report to delete
      ...
      if (findError || !existingReport) {
        console.error('Report not found:', findError);
        showToast(language === 'tr' ? 'Şikayet bulunamadı' : 'Report not found', 'error');
      ...
      if (checkReport) {
        ...
        showToast(language === 'tr' ? 'Şikayet silinemedi, lütfen tekrar deneyin' : 'Could not delete report, please try again', 'error');
        return;
      }
      ...
      // Manually decrease report count
      const newCount = Math.max(0, reportCount - 1);
      setReportCount(newCount);
      
      // Update danger level based on new count
      if (newCount < 5) {
        setDangerLevel('low');
      ...
      showToast(language === 'tr' ? 'Şikayetiniz geri çekildi' : 'Report withdrawn', 'success');
    } catch (error) {
      console.error('Error withdrawing report:', error);
      showToast(language === 'tr' ? 'Şikayet geri çekilemedi' : 'Failed to withdraw report', 'error');
    }
  };
  ...
  const handleBlockUser = async () => {
    ...
    if (!confirm(language === 'tr' ? 'Bu kullanıcıyı engellemek istediğinizden emin misiniz?' : 'Are you sure you want to block this user?')) {
      return;
    }
  ...
  const handleUnblockUser = async () => {
    ...
    try {
      ...
      setIsBlocked(false);
      setShowProfileMenu(false);
      showToast(language === 'tr' ? 'Kullanıcının engeli kaldırıldı' : 'User unblocked', 'success');
    } catch (error) {
      console.error('Error unblocking user:', error);
      showToast(language === 'tr' ? 'Engel kaldırılamadı' : 'Failed to unblock', 'error');
    }
  };
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      if (isFavorite) {
        ...
        setIsFavorite(false);
        showToast(language === 'tr' ? 'Favori arkadaşlardan çıkarıldı' : 'Removed from favorites', 'success');
        ...
        setIsFavorite(true);
        showToast(language === 'tr' ? 'Favori arkadaşlara eklendi' : 'Added to favorites', 'success');
      }
      setShowProfileMenu(false);
    } catch (error) {
      console.error('Error toggling favorite:', error);
      showToast(language === 'tr' ? `İşlem başarısız: ${error instanceof Error ? error.message : 'Bilinmeyen hata'}` : `Operation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, 'error');
    }
  };
  ...
  const handleReportUser = async () => {
    ...
    const reason = prompt(language === 'tr' ? 'Şikayet sebebinizi yazın (isteğe bağlı):' : 'Enter report reason (optional):');
    if (reason === null) return; // User cancelled

    if (!confirm(language === 'tr' ? 'Bu kullanıcıyı şikayet etmek istediğinizden emin misiniz?' : 'Are you sure you want to report this user?')) {
      return;
    }
    ...
    try {
      ...
      if (existingReport) {
        showToast(language === 'tr' ? 'Bu kullanıcıyı zaten şikayet etmişsiniz' : 'You have already reported this user', 'error');
        setShowProfileMenu(false);
        return;
      }
      ...
      console.log('Total reports for user:', totalReports);

      // Get reported user's info and reporter's info for email
      try {
        const [reportedUserResult, reporterResult] = await Promise.all([
          supabase.from('profiles').select('username, full_name').eq('id', profileUserId).single(),
          supabase.from('profiles').select('username, full_name').eq('id', user.id).single(),
        ]);

        const reportedUsername = reportedUserResult.data?.username || reportedUserResult.data?.full_name || 'Unknown';
        const reporterUsername = reporterResult.data?.username || reporterResult.data?.full_name || 'Unknown';

        // Send email notification for EVERY report using hardcoded EmailJS credentials (same as Contact.tsx)
        const emailResponse = await fetch('https://api.emailjs.com/api/v1.0/email/send', {
          ...
          body: JSON.stringify({
            service_id: 'service_6btsv5d',
            template_id: 'template_x7aji5u',
            user_id: 'AdvA9XekMYHYYOhcF',
            template_params: {
              name: 'Submirra Report System',
              email: 'report@submirra.com',
              ...
              message: `🚨 YENİ ŞİKAYET BİLDİRİMİ

🎯 Şikayet Edilen: ${reportedUsername}
👤 Şikayet Eden: ${reporterUsername}
📊 Toplam Şikayet Sayısı: ${totalReports}
📝 Şikayet Sebebi: ${reason || 'Belirtilmedi'}

⚠️ Risk Seviyesi: ${totalReports < 5 ? 'Düşük' : totalReports < 10 ? 'Orta' : totalReports < 20 ? 'Yüksek' : 'Kritik'}

🔗 Profil: https://submirra.com/profile/${profileUserId}`,
            },
          }),
        });
      ...
      // If 20 or more reports, suspend the user
      if (totalReports >= 20) {
      ...
      // Immediately update report count in UI
  ...
  const loadReportInfo = async (userId: string) => {
    try {
      ...
      // 0-4 reports = Low Risk (Safe)
      // 5-9 reports = Medium Risk (Caution)
      // 10-19 reports = High Risk (Warning)
      // 20+ reports = Critical Risk (Danger)
      if (count < 5) {
        setDangerLevel('low');
      ...
      <div className="min-h-screen relative pt-24 pb-16 px-4 md:px-6">
        ...
        <div className="relative max-w-4xl mx-auto z-10">
          <div className="bg-slate-900/50 backdrop-blur-sm border border-purple-500/20 rounded-2xl p-8 text-center">
            ...
            <h2 className="text-2xl font-bold text-white mb-4">{language === 'tr' ? 'Engellediğiniz Kullanıcı' : 'Blocked User'}</h2>
            <p className="text-slate-400 mb-6">
              {language === 'tr' ? 'Bu kullanıcıyı engellemişsiniz. Profilini görüntüleyemezsiniz.' : 'You have blocked this user. You cannot view their profile.'}
            </p>
                  ...
                  {!isOwnProfile && user && (
                    <>
                      ...
                      >
                        {isFavorite ? (
                          <>
                            ...
                            <span>{language === 'tr' ? 'Favorilerden Çıkar' : 'Remove from Favorites'}</span>
                          </>
                        ) : (
                          <>
                            <Star size={16} className="text-yellow-400" />
                            <span>{language === 'tr' ? 'Favorilere Ekle' : 'Add to Favorites'}</span>
                          </>
                        )}
                      </button>
                      ...
                      {isBlocked ? (
                        ...
                        >
                          <Ban size={16} />
                          <span>{language === 'tr' ? 'Engeli Kaldır' : 'Unblock'}</span>
                        </button>
                      ) : (
                        <button
                        ...
                        >
                          <Ban size={16} />
                          <span>{language === 'tr' ? 'Kullanıcıyı Engelle' : 'Block User'}</span>
                        </button>
                      )}
                      ...
                      {hasReported ? (
                        <button
                          ...
                          className="w-full px-3 py-2.5 text-left hover:bg-slate-950/50 transition-colors flex items-center gap-2 text-green-400 text-sm border-t border-slate-700/50"
                        >
                          <Flag size={16} />
                          <span>{language === 'tr' ? 'Şikayeti Geri Çek' : 'Withdraw Report'}</span>
                        </button>
                      ) : (
                        <button
                        ...
                        >
                          <Flag size={16} />
                          <span>{language === 'tr' ? 'Şikayet Et' : 'Report User'}</span>
                        </button>
                      )}
                    </>
                  )}
                  ...
                  {!isEditing && profile?.plan_type && (
                    <span className={`px-2.5 py-1 rounded-lg text-xs font-semibold flex items-center gap-1.5 ${
                      profile.plan_type === 'premium'
                        ...
                        ? 'bg-gradient-to-r from-green-500/20 to-emerald-500/20 border border-green-500/40 text-green-400'
                      ...
                      <Zap size={12} />
                      {profile.plan_type === 'premium' ? 'Premium' : profile.plan_type === 'trial' ? 'Free Trial' : profile.plan_type === 'free' ? 'Free Plan' : 'Standard'}
                    </span>
                  )}
                </div>
                {isEditing ? (

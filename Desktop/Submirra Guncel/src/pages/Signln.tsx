...
export default function SignIn() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        if (!acceptedAge) {
          setError(t.auth.mustAcceptAge || '18 yaşından büyük olduğunuzu onaylamalısınız');
          setLoading(false);
          return;
        }
  ...
  return (
    <div className="min-h-screen relative pt-20 md:pt-24 pb-24 sm:pb-16 px-4 md:px-6 flex items-center">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-start gap-3">
                  <input
                    ...
                    className="mt-1 w-4 h-4 rounded border-purple-500/30 bg-slate-950/50 text-purple-600 focus:ring-purple-500/50 focus:ring-2 cursor-pointer"
                  />
                  <label htmlFor="acceptAge" className="text-slate-300 text-sm md:text-base cursor-pointer flex-1">
                    {t.auth.acceptAge || '18 yaşından büyük olduğumu onaylıyorum'}
                  </label>
                </div>
              </div>
            )}

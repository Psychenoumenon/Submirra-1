...
import { generateCheckoutUrl, LEMON_PRODUCTS } from '../lib/lemonSqueezy';
...
export default function Pricing() {
  ...
  const [currentPlan, setCurrentPlan] = useState<'free' | 'trial' | 'standard' | 'premium' | null>(null);
  ...
  useEffect(() => {
    ...
    const checkStatus = async () => {
      try {
        ...
        if (!profileError && profileData?.trial_used) {
          setTrialUsed(true);
        }
        ...
        if (!subscriptionError && subscriptionData) {
          setCurrentPlan(subscriptionData.plan_type as 'free' | 'trial' | 'standard' | 'premium');
        }
      } catch (error) {
  ...
  const handleSubscribe = async (planType: 'free' | 'standard' | 'premium') => {
    ...
    if (currentPlan === 'premium' && planType === 'standard') {
      showToast(
        language === 'tr'
          ? 'Premium plandan Standard plana geçemezsiniz. Lütfen aboneliğinizi iptal edip yeni bir abonelik başlatın.'
          : 'You cannot downgrade from Premium to Standard. Please cancel your subscription and start a new one.',
        'error'
      );
      return;
    }

    if ((currentPlan === 'standard' || currentPlan === 'premium') && planType === 'free') {
      showToast(
        language === 'tr'
          ? 'Ücretli plandan ücretsiz plana geçmek için ayarlar bölümünden aboneliğinizi iptal edin.'
          : 'To downgrade to free plan, please cancel your subscription from settings.',
        'info'
      );
      return;
    }
    ...
    try {
      if (planType === 'free') {
        ...
        const checkoutId = planType === 'standard' 
          ? LEMON_PRODUCTS.standard.checkoutId 
          : LEMON_PRODUCTS.premium.checkoutId;
        
        const checkoutUrl = generateCheckoutUrl(checkoutId, userEmail, user.id);
      ...
      <div className="relative max-w-7xl mx-auto z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto items-start">
          ...
          <div className="flex flex-col gap-6">
            ...
            >
              {trialUsed ? t.pricing.freeTrial.alreadyUsed : (user ? t.pricing.freeTrial.cta : t.pricing.freeTrial.signUp)}
            </button>
            </div>
          </div>
          ...
          <div className="bg-slate-900/50 backdrop-blur-sm border border-cyan-500/20 rounded-2xl p-5 md:p-6 hover:border-cyan-500/40 transition-all duration-500 hover:shadow-xl hover:shadow-cyan-500/10 md:hover:-translate-y-2 animate-fade-in-delay-2 flex flex-col min-h-[600px] mt-16">
            ...
            <button
              ...
              disabled={isProcessing === 'standard' || currentPlan === 'standard' || currentPlan === 'premium'}
              className={`w-full px-4 py-2.5 rounded-lg text-sm font-semibold transition-all duration-300 flex items-center justify-center gap-2 mt-auto ${
                currentPlan === 'standard' || currentPlan === 'premium'
                  ? 'bg-gradient-to-r from-cyan-600/50 to-blue-600/50 text-white/50 opacity-50 cursor-not-allowed'
            ...
            >
              {isProcessing === 'standard' ? (
                ...
                language === 'tr' ? 'Premium Plandasınız' : 'You have Premium'
              ) : (
          ...
          <div className="bg-gradient-to-br from-pink-500/10 to-purple-500/10 backdrop-blur-sm border-2 border-pink-500/40 rounded-2xl p-5 md:p-6 hover:border-pink-500/60 transition-all duration-500 hover:shadow-2xl hover:shadow-pink-500/20 relative md:hover:-translate-y-3 animate-fade-in-delay-3 flex flex-col min-h-[600px] mt-16">
            ...
            <button
              ...
              disabled={isProcessing === 'premium' || currentPlan === 'premium'}
              className={`w-full px-4 py-3 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center gap-2 mt-auto ${
                currentPlan === 'premium'
                  ? 'bg-gradient-to-r from-pink-600/50 to-purple-600/50 text-white/50 opacity-50 cursor-not-allowed'

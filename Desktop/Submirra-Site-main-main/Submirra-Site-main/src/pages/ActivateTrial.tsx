...
export default function ActivateTrial() {
  ...
  const activateTrial = async () => {
    ...
    try {
      // TEMPORARILY DISABLED: IP check for testing purposes
      // TODO: Re-enable this after testing by uncommenting the code below
      /*
      // Get user's IP address
      ...
      */
      ...
      // Get IP for recording purposes only (not for checking)
      const userIP = await getUserIP();
      ...
      console.log('Trial activated successfully in profiles:', data);

      // Also update/create subscription using RPC function
      const { error: subscriptionError } = await supabase.rpc('update_subscription_plan', {
        p_user_id: user.id,
        p_plan_type: 'trial'
      });
      ...
      if (subscriptionError) {
        ...
        // Don't throw error here, profile is already updated
      } else {
        console.log('Subscription updated successfully to trial');
      }

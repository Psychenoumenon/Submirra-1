...
export default function Messages() {
  ...
  useEffect(() => {
    try {
      ...
      try {
        // Load cached conversations from localStorage first for instant display
        const conversationsKey = `conversations_${user.id}`;
        const cachedConversations = localStorage.getItem(conversationsKey);
      ...
      // conversationsKey already defined at the beginning of loadConversations
      setConversations(prev => {
        // If this is the first load and we have conversations, use them
        if (prev.length === 0 && conversationsList.length > 0) {
          try {
            localStorage.setItem(conversationsKey, JSON.stringify(conversationsList));
          } catch (e) {
        ...
        try {
          localStorage.setItem(conversationsKey, JSON.stringify(mergedList));
        } catch (e) {
          console.error('Failed to save conversations to localStorage:', e);
        }
    ...
    // Fallback: Poll for new messages every 3 seconds for faster updates
    ...
    const pollInterval = setInterval(() => {
      ...
      // Reload conversations to keep unread count updated
      loadConversations();
    }, 3000);
  ...
  const deleteConversation = async () => {
    ...
    try {
      ...
      setConversations(prev => {
        const filtered = prev.filter(conv => conv.user_id !== conversationToDelete);
        ...
        try {
          ...
          console.error('Failed to update cached conversations:', e);
        }
